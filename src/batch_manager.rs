...
impl BatchIndexManager {
    ...
    pub async fn get_batch_status(&self, batch_id: &str) -> Option<(usize, usize)> {
        ...
        batches.get(batch_id).map(|batch| {
            (batch.received.len(), batch.total)
        })
    }
...
mod tests {
    ...
    async fn test_batch_manager_basic_flow() {
        ...
        // Start batch
        manager.start_batch("test_batch".to_string(), 3).await.unwrap();
    ...
    async fn test_duplicate_index_handling() {
        ...
        manager.add_to_batch("test".to_string(), 0, bookmark.clone(), "Content".to_string())
            .await.unwrap();
        
        // Second add should be ignored
        manager.add_to_batch("test".to_string(), 0, bookmark, "Content2".to_string())
            .await.unwrap();
