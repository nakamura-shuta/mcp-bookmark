...
- 📄 **PDF の完全サポート** - PDF.js でクライアント側テキスト抽出、全ページ検索可能
...
- **PDF 全文検索**: PDF.js による多ページ PDF のテキスト抽出・検索
...
ローカルに保存された PDF ファイルをインデックス化する場合：

1. **Chrome でローカル PDF を開く**:
   - Chrome のアドレスバーに `file:///` と入力するか、PDF ファイルを Chrome にドラッグ＆ドロップ
   - または、Finder で PDF を右クリック → 「このアプリケーションで開く」→「Google Chrome」

2. **ブックマークに追加**:
   - PDF が Chrome で開いたら、⌘+D（Mac）または Ctrl+D（Windows）でブックマーク
   - 適切なフォルダ（インデックス化対象のフォルダ）に保存

3. **Chrome 拡張機能でインデックス化**:
   - 拡張機能のポップアップで対象フォルダを選択
   - 「Index Selected Folder」をクリック
   - PDF.js により自動的に全ページのテキストが抽出・インデックス化されます

4. **検索で利用**:
   - 例：「ブックマークから Linux カーネルについて検索して」
   - PDF 内の全ページから関連情報を検索可能
